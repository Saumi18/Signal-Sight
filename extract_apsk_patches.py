...
def extract_apsk_patch(spectrogram, patch_shape=(64, 32), lam=0.2):
    ...
    for y in range(height - ph + 1):
        for x in range(width - pw + 1):
            patch = spectrogram[y:y+ph, x:x+pw].flatten()
            var = np.var(patch)
...
for fname in spect_files:
    ...
    patch, coords = extract_apsk_patch(Sxx, patch_shape=patch_shape, lam=lam)
    patch_fname = fname.replace('spectrogram_', 'patch_')
    np.save(os.path.join(output_dir, patch_fname), patch)

print(f"Extracted {len(spect_files)} APSK patches to {output_dir}")
