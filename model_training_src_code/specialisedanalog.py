...
class AnalogSpecialistDataset(Dataset):
    def __init__(self, npz_path, transform=None):
        print(f"Loading and preparing data for Analog Specialist from {npz_path}...")
        ...
        self.analog_mods = ['AM-SSB-WC', 'AM-SSB-SC', 'AM-DSB-WC', 'AM-DSB-SC', 'FM']
        analog_indices_in_special = {self.base_mod_classes.index(m): i for i, m in enumerate(self.analog_mods)}
        
        Y_router = data['Y_router']
        analog_samples_mask = Y_router[:, 0] == 1
        X_spectrograms_filtered = data['X_spectrograms'][analog_samples_mask]
        Y_special_filtered = data['Y_special'][analog_samples_mask]
        ...
        for i in range(len(X_spectrograms_filtered)):
            ...
            found_label = False
            for global_idx, class_idx in analog_indices_in_special.items():
...
def main():
    ...
    FAMILY_NAME = "analog"
    CLASS_NAMES = ['AM-SSB-WC', 'AM-SSB-SC', 'AM-DSB-WC', 'AM-DSB-SC', 'FM']    
    ...
    dataset = AnalogSpecialistDataset(npz_path=NPZ_DATA_PATH)
    ...
    try:
        report = classification_report(test_labels, test_preds, target_names=CLASS_NAMES, labels=range(NUM_CLASSES), output_dict=True, zero_division=0)
