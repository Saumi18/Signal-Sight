...
class QAMSpecialistDataset(Dataset):
    def __init__(self, npz_path, transform=None):
        print(f"Loading and preparing data for QAM Specialist from {npz_path}...")
        ...
        self.qam_mods = ['4ASK', '8ASK', '16QAM', '32QAM', '64QAM', '128QAM', '256QAM']
        qam_indices_in_special = {self.base_mod_classes.index(m): i for i, m in enumerate(self.qam_mods)}
        
        Y_router = data['Y_router']
        qam_samples_mask = Y_router[:, 2] == 1 
        X_spectrograms_filtered = data['X_spectrograms'][qam_samples_mask]
        Y_special_filtered = data['Y_special'][qam_samples_mask]
        ...
        for i in range(len(X_spectrograms_filtered)):
            ...
            found_label = False
            for global_idx, class_idx in qam_indices_in_special.items():
...
def main():
    ...
    FAMILY_NAME = "qam"
    CLASS_NAMES = ['4ASK', '8ASK', '16QAM', '32QAM', '64QAM', '128QAM', '256QAM']
    ...
    dataset = QAMSpecialistDataset(npz_path=NPZ_DATA_PATH)
