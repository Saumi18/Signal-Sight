...
X_pure = []
for _ in range(pure):
    ind = np.random.randint(len_family['qam'])
    X_qam.append(family_X['qam'][ind])
    Y_qam.append(family_Y['qam'][ind])

X_mix2 = []
for _ in range(mix_2):
    chosen_fam_arr = np.random.choice(family_names, size=1, replace=False)
    chosen_fam_arr = np.append(chosen_fam_arr, 'qam')
    ...
    X_qam.append(mix_sig)
    Y_qam.append(family_Y[chosen_fam_arr[1]][ind2])
...
for _ in range(mix_3):
    ...
    X_qam.append(mix_sig)
    Y_qam.append(family_Y[chosen_fam_arr[2]][ind3])
...
for _ in range(mix_3):
    ...
    X_qam.append(mix_sig)
    Y_qam.append(family_Y[chosen_fam_arr[3]][ind4])


X_qam = np.array(X_qam)
Y_qam = np.array(Y_qam)
