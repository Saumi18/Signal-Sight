...
def extract_phase_patch(spectrogram, patch_shape=(64, 32), lam=0.2):
...
input_dir = '/home/taksh/Summer_RAID_Project/spectrograms/phase'
output_dir = '/home/taksh/Summer_RAID_Project/patches/phase_patches'
...
for fname in spect_files:
    ...
    patch, coords = extract_phase_patch(Sxx, patch_shape=patch_shape, lam=lam)
    patch_fname = fname.replace('spectrogram_', 'patch_')
    np.save(os.path.join(output_dir, patch_fname), patch)

print(f"Extracted {len(spect_files)} phase patches to {output_dir}")