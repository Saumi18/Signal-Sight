...
X_pure = []
for _ in range(pure):
    ind = np.random.randint(len_family['phase'])
    X_phase.append(family_X['phase'][ind])
    Y_phase.append(family_Y['phase'][ind])

X_mix2 = []
for _ in range(mix_2):
    chosen_fam_arr = np.random.choice(family_names, size=1, replace=False)
    chosen_fam_arr = np.append(chosen_fam_arr, 'phase')
    ...
    X_phase.append(mix_sig)
    Y_phase.append(family_Y[chosen_fam_arr[1]][ind2])
...
for _ in range(mix_3):
    ...
    X_phase.append(mix_sig)
    Y_phase.append(family_Y[chosen_fam_arr[2]][ind3])
...
for _ in range(mix_3):
    ...
    X_phase.append(mix_sig)
    Y_phase.append(family_Y[chosen_fam_arr[3]][ind4])
